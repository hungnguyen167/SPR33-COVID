...
#### Results for Gov. Intervention Measurement - Table A1
...
#### Results for Public Behaviors Measurement - Table A2
...
##### Table A5
```{r a5}
m_coef <- function(formula, data, indices) {
val <- data[indices,] # selecting sample with boot 
fit <- lm(formula, data=val)
return(summary(fit)$coefficients)
} 

m51 <- boot::boot(data = df, statistic = m_coef, R = 1000, formula = behave ~ gap1a)
 
m52 <- boot::boot(data = df, statistic = m_coef, R = 1000, formula = behave  ~  gap1a + chi2) 

m53 <-  boot::boot(data = df, statistic = m_coef, R = 1000, formula = behave  ~ gap1a + chi2 + dpm2)

m54 <-  boot::boot(data = df, statistic = m_coef, R = 1000, formula =behave  ~ gap1a + chi2 + dpm2 + gini_disp)

m55 <-  boot::boot(data = df, statistic = m_coef, R = 1000, formula = behave ~ gap1a + chi2 + dpm2 + gini_disp + soc_spend)

m56 <-  boot::boot(data = df, statistic = m_coef, R = 1000, formula = behave ~ gap1a + chi2 + dpm2 + gini_disp + soc_spend  + gdp)
...
m61 <- lm(behave ~ gap1, data = df)

m62 <- lm(behave ~ gap1 + chi2, data = df)

m63 <- lm(behave ~ gap1 + chi2 + dpm2, data = df)

m64 <- lm(behave ~ gap1 + chi2 + dpm2 + gini_disp, data = df)

m65 <- lm(behave ~ gap1 + chi2 + dpm2 + gini_disp + soc_spend, data = df)

m66 <- lm(behave ~ gap1 + chi2 + dpm2 + gini_disp + soc_spend + gdp, data = df)



tab_model(m61, m62, m63, m64, m65, m66, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M61", "M62","M63", "M64","M65", "M66"), pred.labels = c("Intercept",  "Discrepancy Score from the Outbreak Stage", "Current Gov. Intervention", "Deaths per Million", "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"),show.aic = TRUE)
```


#### Figure 3 - Residuals and Behavior Compared
...
sc2 <- ggscatter(df, x = "gap1a", y = "chi2", conf.int =  TRUE, size = 5) +
    stat_cor(method = "pearson", size = 10, label.x = 0.08, label.y = -2) +
    geom_smooth(method = lm, se = TRUE, fullrange = FALSE, aes(group = 1), color = "blue") +
    geom_text_repel(aes(label = iso), size = 7) +
    scale_color_discrete(name = "Country (ISO)") +
    xlab(label = "Discrepancy Score") +
    ylab(label = "Government Intervention in the Reaction Stage") +
    theme(axis.title = element_text(size = 24,face = "bold"),axis.text = element_text(size = 12))
...
knitr::include_graphics(wdir("results/scatplots.png"))
```
