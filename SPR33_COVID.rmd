---
title: 'Locked Down or Locked In? Institutionalized Public Preferences and Pandemic Policy Feedback in 32 Countries - Online Appendix'
output:
  html_document:
    df_print: paged
---
...
tab_model(m11, m12, m13, m14, m15, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M11", "M12","M13", "M14","M15"), pred.labels = c("Intercept", "Deaths per Million", "Public Preferences", "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"), show.std =  TRUE)
...
htmlTable(tab1.2)
## A categorical variable of the discrepancy score. 0.88 = SD of gap1
df$gap1_cat <- as.factor(ifelse(df$gap1 > 0.88, 1, 0)) 
```

Because dpm1 and dpm2 are highly correlated (see Correlation Table), constructing the discrepancy score from a model with dpm1 as a predictor and using this score as one of the main predictors in the public reaction stage would introduce multicollinearity. Therefore, we construct an alternative discrepancy score which is the difference between predicted values of a model in which chi1 is regressed on gov_author only and actual values of chi1. This alternative score is used for our preferred models.
...
m11a <- lm(chi1 ~ gov_author, data = df)
df$chi1_pa <- predict.lm(m11a, newdata = df)
...
## A categorical variable of the discrepancy score. 0.95 = SD of gap1a
df$gap1a_cat <- as.factor(ifelse(df$gap1a > 0.95, 1, 0))
...
#### Public Reaction Stage (March 20th - April 7th)
...
m22 <- lm(behave  ~ dpm2 + chi2 + gap1a, data = df)

m23 <- lm(behave  ~ dpm2 +  chi2 + gap1a + gini_disp, data = df)

m24 <- lm(behave ~ dpm2 +  chi2 + gap1a + soc_spend, data = df)

m25 <- lm(behave ~ dpm2 + chi2 + gap1a +  gdp, data = df)

m26 <- lm(behave ~ dpm2 + chi2 + gap1a  + gini_disp + soc_spend + gdp, data = df)



tab_model(m21, m22, m23, m24, m25, m26, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M21", "M22","M23", "M24","M25", "M26"), pred.labels = c("Intercept", "Deaths per Million", "Current Gov. Intervention", "Discrepancy Score from the Outbreak Stage",  "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"),show.aic = TRUE)
```
...
m32 <- lm(behave  ~ dpm2 + chi2 + gap1a_dic, data = df)

m33 <- lm(behave  ~ dpm2 +  chi2 + gap1a_dic  + gini_disp, data = df)

m34 <- lm(behave ~ dpm2 +  chi2 + gap1a_dic  + soc_spend, data = df)

m35 <- lm(behave ~ dpm2 + chi2 + gap1a_dic + gdp, data = df)

m36 <- lm(behave ~ dpm2 + chi2 + gap1a_dic + gini_disp + soc_spend + gdp, data = df)
...
m51 <- lm(behave ~ dpm2 + chi2 , data = df)

m52 <- lm(behave  ~ dpm2 + chi2 + gap1, data = df)

m53 <- lm(behave  ~ dpm2 +  chi2 + gap1 + gini_disp, data = df)

m54 <- lm(behave ~ dpm2 +  chi2 + gap1 + soc_spend, data = df)

m55 <- lm(behave ~ dpm2 + chi2 + gap1 +  gdp, data = df)

m56 <- lm(behave ~ dpm2 + chi2 + gap1  + gini_disp + soc_spend + gdp, data = df)



tab_model(m51, m52, m53, m54, m55, m56, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M21", "M22","M23", "M24","M25", "M26"), pred.labels = c("Intercept", "Deaths per Million", "Current Gov. Intervention", "Discrepancy Score from the Outbreak Stage",  "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"),show.aic = TRUE)
```
...
m41 <- lm(behave ~ dpm2 + chi2, data = df)

m42 <- lm(behave ~ dpm2 + gap2 + chi2, data = df)

m43 <- lm(behave ~ dpm2 + gap2 + chi2 + gini_disp , data = df)

m44 <- lm(behave ~ dpm2 + gap2 + chi2 + soc_spend , data = df)

m45 <- lm(behave ~ dpm2 + gap2 + chi2 + gdp, data = df)

m46 <- lm(behave ~ dpm2 + gap2 + chi2 + gini_disp + soc_spend + gdp, data = df)



tab_model(m41, m42, m43, m44, m45, m46, p.style = "stars", p.threshold = c(0.10, 0.05, 0.01), show.ci = F,  dv.labels = c("M41", "M42","M43", "M44","M45", "M46"), pred.labels = c("Intercept", "Deaths per Million", "Current Gov. Intervention", "Discrepancy Score from the Outbreak Phase", "Disposable Income Inequality", "Social Spending", "GDP (k$, per capita)"))
```


#### Figure 2 - Residuals and Behavior Compared
...
agg_png(filename = "results/Fig2.png", res = 144, height = 800, width = 1000)
...
agg_png(wdir("results/figa1.png"),width = 1600,height = 900)
...
knitr::include_graphics("results/figa1.png")

```

#### Sample Representativeness - A2 to A5
...
agg_png(wdir("results/figa4.png"),width = 1600,height = 900)
...
agg_png(wdir("results/figa5.png"),width = 1600,height = 900)
